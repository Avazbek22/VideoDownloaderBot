...
def _warn_if_missing_node() -> None:
    if shutil.which("node") is None:
        print("[YouTube] Warning: nodejs not found in PATH; YouTube extraction may fail.")
...
def _log_youtube_attempt(message: str) -> None:
...
def _download_and_send(job: Dict[str, Any]) -> None:
    ...
    if is_youtube and mode != "audio":
        format_candidates = _youtube_format_attempts()
    else:
        format_candidates = [format_spec]
    ...
    try:
        ...
        for attempt_idx, attempt_format in enumerate(format_candidates, start=1):
            ...
            if is_youtube:
                _warn_if_missing_node()
                _log_youtube_attempt(f"Attempt {attempt_idx}/2 using format: {attempt_format}")
            ...
            if is_youtube:
                ydl_opts.update(_youtube_ydl_opts())
                if mode != "audio" and attempt_format == format_candidates[0]:
            ...
            if is_youtube:
                ...
                _log_youtube_attempt(f"Downloaded {ext or 'unknown'} file, size {_fmt_bytes(final_size)}")
